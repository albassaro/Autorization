...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if (empty($login)) {
        setErrorMessage(fieldName:'login', textMessage:'Ошибка. Пустое поле с логином');
    }
    if (empty($password)) {
        setErrorMessage(fieldName: 'password', textMessage:'Ошибка. Пустое поле с паролем');
    }
    ...
    if ($CSRF == $_SESSION['CSRF-token']) {
        ...
        if (!$user){
            setErrorMessage(fieldName:'login', textMessage:"Ошибка. Пользователь с таким логином не зарегистрирован");
            // Редирект на страницу формы если есть ошибки
            redirectWithError($login,$password,'authorise.php');
        }

        if(!password_verify($password , $user['user_password'])){
            setErrorMessage(fieldName:'password', textMessage:"Ошибка. Неверный пароль");
            // Редирект на страницу формы если есть ошибки
            redirectWithError($login,$password,'authorise.php');
        }
